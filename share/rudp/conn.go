...
func NewConn(conn *net.UDPConn, rudp *Rudp) *RudpConn {
	con := &RudpConn{conn: conn, rudp: rudp,
		...
		SendTick: make(chan time.Duration, 2),
	}
	go con.run()
	return con
}
...
func NewUnConn(conn *net.UDPConn, remoteAddr *net.UDPAddr, rudp *Rudp, close func(string)) *RudpConn {
	con := &RudpConn{conn: conn, rudp: rudp, SendTick: make(chan time.Duration, 2),
		recvChan: make(chan []byte, 1<<16), recvErr: make(chan error, 2),
		sendChan: make(chan []byte, 1<<16), sendErr: make(chan error, 2),
...
func (rc *RudpConn) Close() error {
	...
	log.Debug("来自%s的RUDP连接已关闭", rc.remoteAddr.String())
	return err
}
...
func (rc *RudpConn) connectedRecvLoop() {
...
func (rc *RudpConn) unconnectedRecvLoop() {
	data := make([]byte, MAX_PACKAGE)
	for {
		select {
		case bts := <-rc.in:
	...
	for {
		...
		sendOut:
			...
			if num > 1 {
				...
				log.Debug("send package num %v,sz %v, %v/s,local %v,remote %v",
...
func (rc *RudpConn) run() {
	if autoSend && sendTick > 0 {
		go func() {
			tick := time.Tick(sendTick) // 默认sendTick=10ms
			for {
				select {
				case <-tick:
					rc.SendTick <- 1 * sendTick
				}
			}
		}()
	}
	go func() {
		if rc.Connected() {
			rc.connectedRecvLoop()
		} else {
			rc.unconnectedRecvLoop()
		}
	}()
	rc.sendLoop()
}
